...
| **O `rem` do HTML colado vira px, e a base é escolha da página.** Padrão: 16 px, a página fica exatamente como aparece ao abrir o arquivo sozinho no navegador (o visual aprovado). Opção "Seguir o tamanho de texto do tema" (Configurações da página): o servidor lê as folhas do tema (`html{font-size:calc(var(--font-body-scale)*62.5%)}` → 10 px) e converte com esse número — canvas, pré-visualização e publicação iguais (I1); a barra de status avisa ("1 rem = 10 px, como no tema da loja"). Autor que declara a própria raiz (`html{font-size:…}`) ganha das duas. | `Page.remFromTheme`, `theme-style.server.ts` `rootPx`, `html-optimize.ts` `themeRootPx`/`authorRootPx`, `compile({ rootPx })` | Medido em 22/09: a 10 px o h1 sai 38 px em vez de 60,8, e preço + botão entram na primeira tela do celular (92 no PageSpeed); a 16 px é o visual que o Miguel preferiu. Os dois publicam; a nota depende mais do que está acima da dobra do que da base |
...
| Seções de um HTML colado (a regra da linha `content-visibility` acima só alcança os blocos de topo do documento, não o que vive dentro de um bloco HTML) | — | O autor põe `content-visibility:auto; contain-intrinsic-size:auto 600px` nas próprias seções abaixo da dobra (regra 18 do `PROMPT_GERADOR_LP.md`). Medido na Mini Plancha (simulação, CPU 4×, 4 rodadas): FCP 796 → 700 ms, long tasks 1507 → 1284 ms, LCP igual. Efeito colateral pago: a rolagem suave até a oferta errava o alvo em ~1000 px (as seções crescem no meio do caminho) — o clique agora calcula a página inteira antes de medir (classe `all-laid`) |
| Google Fonts × fonte do sistema | (dentro do aviso da folha) | Medido na Mini Plancha (mesma simulação): FCP e LCP empatam (a `display=optional` já evita a espera); só os long tasks caem ~250 ms e saem 75 KB + 2 conexões. É escolha visual do dono: a versão com fonte do sistema está pronta em `docs/lps/mini-plancha-lp-fonte-sistema.html` |
